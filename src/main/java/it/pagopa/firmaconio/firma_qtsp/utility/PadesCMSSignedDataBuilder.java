...
public class PadesCMSSignedDataBuilder extends CMSSignedDataBuilder {
	 ...
	 * The {@code CertificateVerifier} is used to find the trusted certificates.
	 *
	...
	@Override
	public CMSSignedDataGenerator createCMSSignedDataGenerator(CAdESSignatureParameters parameters,
			ContentSigner contentSigner, SignerInfoGeneratorBuilder signerInfoGeneratorBuilder,
			CMSSignedData originalSignedData) throws DSSException {

		return super.createCMSSignedDataGenerator(parameters, contentSigner, signerInfoGeneratorBuilder,
				originalSignedData);
	}

	/**
	 * @param parameters the parameters of the signature containing values for the
	 *                   attributes
	 * @return a SignerInfoGeneratorBuilder that generate the signed and unsigned
	 *         attributes according to the CAdESLevelBaselineB and
	 *         PAdESLevelBaselineB
	 */
	public SignerInfoGeneratorBuilder getSignerInfoGeneratorBuilder(final PAdESSignatureParameters parameters,
		...
		signerInfoGeneratorBuilder = signerInfoGeneratorBuilder.setSignedAttributeGenerator(
				params -> padesProfileB.getSignedAttributes(params, cadesLevelBaselineB, parameters, messageDigest));

		signerInfoGeneratorBuilder = signerInfoGeneratorBuilder
				.setUnsignedAttributeGenerator(params -> padesProfileB.getUnsignedAttributes());
